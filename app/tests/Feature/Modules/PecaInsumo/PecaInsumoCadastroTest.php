...
class PecaInsumoCadastroTest extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->assertDatabaseEmpty('peca_insumo'); // Corrigir o nome da tabela
        ...
        $this->payload = [
            ...
            'qtd_atual' => 100,
            'qtd_segregada' => 5,
            'status' => 'ativo' // Usar valor aceito pelo sistema
        ];
    }
    ...
    {
        $this->postJson('/api/peca_insumo', $this->payload)->assertCreated();

        $novo = $this->payload;
        $novo['descricao'] = 'Filtro Diferente';

        $response = $this->postJson('/api/peca_insumo', $novo);
        $response->assertCreated();
    }
