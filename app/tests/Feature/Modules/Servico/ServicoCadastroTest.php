...
class ServicoCadastroTest extends TestCase
{
    ...
    {
        $response = $this->withAuth()->postJson('/api/servico', $this->payload);
    ...
    {
        // Primeiro cadastro
        $this->withAuth()->postJson('/api/servico', $this->payload)->assertCreated();

        // Segundo com mesma descrição
        $novo = $this->payload;
        $novo['descricao'] = 'Pintura';

        $response = $this->withAuth()->postJson('/api/servico', $novo);
